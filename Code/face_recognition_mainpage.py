...
class face:
    def __init__(self, root):
        ...
        # for title in bg_img
        title_lbl = Label(self.root, text="FACE RECOGNITION", font=("times new roman", 40, "bold"), bg="darkblue",
                          fg="white")
        title_lbl.place(x=0, y=0, width=1530, height=45)

        # topright_side_img
        right_side_img = Image.open(
        ...
        f1_lbl.place(x=0, y=55, width=690,
                     height=790)  # it is for decide the position of image in window by using place method

        # big_side_img
        big_side_img = Image.open(
            r"C:\Users\kumar\Desktop\college_images\4.jpg ")  # for calling the images fromfolder to window
        ...
        f2_lbl.place(x=690, y=55, width=810,
                     height=790)  # it is for decide the position of image in window by using place method

        # face_recogniton Button
        face_recogniton_btn = Button(f2_lbl, width=250, text="Face Recogniton Button", command=self.face_recog,
    ...
    def mark_Attendence(self, fetched_rollno_data, fetched_name_data, fetched_department_data, fetched_course_data):
        ...
        with open(filename, "r+", newline="") as f:
            ...
            if fetched_rollno_data not in name_list:
                ...
                f.write(
                    f"{fetched_rollno_data},{fetched_name_data},{fetched_department_data},{fetched_course_data},{date_string},{time_string},Present\n")
        ...
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf, sent_names):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 70:
                    ...
                    self.mark_Attendence(fetched_rollno_data, fetched_name_data, fetched_department_data,
                                         fetched_course_data)
                    cv2.putText(img, f"RollNO:{fetched_rollno_data}", (x, y - 100), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (255, 255, 255), 3)

                    cv2.putText(img, f"Name:{fetched_name_data}", (x, y - 75), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (255, 255, 255), 3)
                    self.send_to_arduino(fetched_name_data)
                    cv2.putText(img, f"Deparment:{fetched_department_data}", (x, y - 45), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (255, 255, 255), 3)
                    cv2.putText(img, f"Group:{fetched_group_data}", (x, y - 15), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (255, 255, 255), 3)
                    cv2.putText(img, f"Course:{fetched_course_data}", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (255, 255, 255), 3)
                    # self.mark_Attendence(fetched_rollno_data,fetched_name_data,fetched_department_data,fetched_course_data)
