...
class Train_dataset:
    def __init__(self, root):
        ...
        # for title in bg_img
        title_lbl = Label(self.root, text=" Train Data Set", font=("times new roman", 40, "bold"), bg="white",
                          fg="darkgreen")
        ...
        f1_lbl.place(x=0, y=55, width=1500,
                     height=350)  # it is for decide the position of image in window by using place method

        # Trainig Button
        Traing_btn = Button(self.root, width=1500, text="Train DataSet ", command=self.train_clessifier,
                            font=("times new roman", 30, "bold"), bg="darkblue", fg="white")
        Traing_btn.place(x=0, y=410, width=1500, height=82)  #

        # bottom image
        bottom_img = Image.open(
            r"C:\Users\kumar\Desktop\college_images\bottomtrain.jpg")  # for calling the images fromfolder to window
        ...
        f1_lbl.place(x=0, y=500, width=1500,
                     height=350)  # it is for decide the position of image in window by using place method

    def train_clessifier(self):
        data_dir = ("DATAIMG")
        path = [os.path.join(data_dir, file) for file in
                os.listdir(data_dir)]  ###it is a list comprihension way for using loop and list
